<template>
  <div class="search-body">
    ...
    <div class="search-group">
      <i-row>
        <i-col span="19">
          <input v-model="keyWord" class="search-input" type="text" placeholder="请输入企业名称、案号">
        </i-col>
        <i-col span="5">
    ...
    <div v-if="isGoSearch" class="search-result">
      ...
      >
        <i-row i-class="lh50">
          ...
          <i-col span="19" i-class="font-click">{{item.decisionTitle}}</i-col>
        </i-row>
        <i-row i-class="lh50">
          <i-col span="4" offset="1">案号：</i-col>
          <i-col span="19" i-class="sub-content">{{item.caseCode}}</i-col>
        </i-row>
        <i-row i-class="lh50">
          <i-col span="6" offset="1">执行机构：</i-col>
          <i-col span="17" i-class="sub-content">{{item.executiveCourt}}</i-col>
        </i-row>
        <i-row i-class="lh50">
          <i-col span="4" offset="1">类型：</i-col>
          <i-col span="19" i-class="sub-content">{{item.caseType}}</i-col>
        </i-row>
        <i-row i-class="lh50">
          <i-col span="4" offset="1">时间：</i-col>
          <i-col span="19" i-class="sub-content">{{item.pubDate}}</i-col>
        </i-row>
        <i-row i-class="lh50">
          <i-col span="7" offset="1">裁决书内容：</i-col>
          <i-col span="16" i-class="sub-content">点击查看裁决书内容</i-col>
        </i-row>
      </div>
    </div>
    <!-- 没有搜索到数据显示的部分 -->
  ...
  methods: {
    ...
    goSearch(init) {
      ...
      if (this.keyWord === "") {
        ...
        utils.showLoading();
        this.$http
          .get(config.courtLawByPageSearch, {
          ...
          .then(res => {
            ...
            if (res.data.code == 0) {
              ...
              if (res.data.data.counts > 0) {
                ...
                this.hasData = true;
                const searchList = res.data.data.laws.map(function(item) {
                  if (item.pubDate) {
                    item.pubDate = utils.formatTime(new Date(item.pubDate), 0);
                  }
                  return item;
                });
