    ...
    init(init) {
      ...
      this.$http.get(config.riskItemDetail, param).then(res => {
        if (res.data.reason == "ok") {
            console.log(res)
          const result = res.data.result.dataList.map(item=>{
              if(res.data.result.type == 6){
                  item.publishDate = formatTime(new Date(item.publishDate),0)
              }
              if(res.data.result.type == 7){
                  item.putDate = formatTime(new Date(item.putDate),0)
              }
              if(res.data.result.type == 8){
                  item.submittime = formatTime(new Date(item.submittime),0)
              }
              if(res.data.result.type == 8){
                  item.putDate = formatTime(new Date(item.putDate),0)
              }
              if(res.data.result.type == 13){
                  item.startdate = formatTime(new Date(item.startdate),0)
              }
              return item
          });
