<template>
  <div class="default-bg min-height100">
    ...
    <div v-else>
      ...
      >
        <div class="left year">{{item.baseInfo.reportYear}}年度</div>
...
export default {
  ...
  methods: {
    annualreportDetail(item) {
      ...
      wx.navigateTo({
        url: "/pages/companyInfoDepthPackage/pages/comBascInfo/qynbDetail/main"
      });
    },
    init(init) {
      if (init) {
      ...
      this.$http
        .get(config.companyName, {
          uri: config.annualreportDe + this.companyName,
