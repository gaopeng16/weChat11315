<template>
  <div class="search-body">
    ...
    <div class="search-group">
      <i-row>
        <i-col span="19">
          <input v-model="keyWord" class="search-input" type="text" placeholder="请输法人名或公司名称">
        </i-col>
        <i-col span="5">
    ...
    <div v-if="isGoSearch" class="search-result">
      ...
      >
        <i-row i-class="lh50">
          <i-col span="1" i-class="font-click">|</i-col>
          <i-col span="4">案号：</i-col>
          <i-col span="19" i-class="font-click">{{item.caseCode}}</i-col>
        </i-row>
        <i-row i-class="lh50">
          <i-col span="6" offset="1">执行法院：</i-col>
          <i-col span="17" i-class="sub-content">{{item.courtName}}</i-col>
        </i-row>
        <i-row i-class="lh50">
          <i-col span="4" offset="1">省份：</i-col>
          <i-col span="19" i-class="sub-content">{{item.areaName}}</i-col>
        </i-row>
        <i-row i-class="lh50">
          <i-col span="4" offset="1">法人：</i-col>
          <i-col span="19" i-class="sub-content">{{item.businessEntity}}</i-col>
        </i-row>
        <i-row i-class="lh50">
          <i-col span="6" offset="1">失信类型：</i-col>
          <i-col span="17" i-class="sub-content">{{item.disruptTypeName}}</i-col>
        </i-row>
        <i-row i-class="lh50">
          <i-col span="12" offset="1">生效法律文书确定的义务：</i-col>
          <i-col span="11" i-class="sub-content">{{item.duty}}</i-col>
        </i-row>
        <i-row i-class="lh50">
          <i-col span="7" offset="1">执行依据文号：</i-col>
          <i-col span="16" i-class="sub-content">{{item.gistId}}</i-col>
        </i-row>
        <i-row i-class="lh50">
          <i-col span="9" offset="1">做出执行依据单位：</i-col>
          <i-col span="14" i-class="sub-content">{{item.gistUnit}}</i-col>
        </i-row>
        <i-row i-class="lh50">
          <i-col span="10" offset="1">被执行人的履行状况：</i-col>
          <i-col span="13" i-class="sub-content">{{item.performance}}</i-col>
        </i-row>
        <i-row i-class="lh50">
          <i-col span="6" offset="1">发布时间：</i-col>
          <i-col span="17" i-class="sub-content">{{item.publishDate}}</i-col>
        </i-row>
        <i-row i-class="lh50">
          <i-col span="6" offset="1">立案时间：</i-col>
          <i-col span="17" i-class="sub-content">{{item.regDate}}</i-col>
        </i-row>
        <i-row i-class="lh50">
          <i-col span="6" offset="1">企业名称：</i-col>
          <i-col span="17" i-class="font-click">{{item.disruptTypeName}}</i-col>
        </i-row>
      </div>
    </div>
    <!-- 没有搜索到数据显示的部分 -->
          ...
          .then(res => {
            ...
            if (res.data.code == 0) {
              ...
              if (res.data.data.counts > 0) {
                ...
                this.hasData = true;
                const searchList = res.data.data.list.map(function(item) {
                  if (item.publishDate) {
                    item.publishDate = utils.formatTime(
                      new Date(item.publishDate),
                      0
                    );
                  }
                  if (item.regDate) {
                    item.regDate = utils.formatTime(new Date(item.regDate), 0);
                  }
                  if (item.performance == "" || item.performance == undefined) {
                    item.performance = "企业选择不公开";
                  }
                  return item;
                });
