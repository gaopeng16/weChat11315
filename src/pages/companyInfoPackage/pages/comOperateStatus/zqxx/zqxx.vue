<template>
  <div class="default-bg min-height100">
    <ToDepthSearch
      :url="'/pages/companyInfoDepthPackage/pages/comOperateStatus/zqxx/main'"
      v-if="total <= 0"
    ></ToDepthSearch>
    <div v-else>
      <DepthSearch :url="'/pages/companyInfoDepthPackage/pages/comOperateStatus/zqxx/main'"></DepthSearch>
      ...
      >
        <BlockTitle :bt="bt" :title="item.bondTradeTime" :hasBt="true"></BlockTitle>
        <div class="lh50">
          债券名称：
...
export default {
  ...
  methods: {
    init(init) {
      ...
      this.$http
        ...
        .then(res => {
          // console.log("res-->", res);
          if (res.data.code == 0) {
            const result = res.data.data.bond;
            if (
    ...
    operateStatusBondDetail(item) {
      ...
      wx.navigateTo({
        url: "/pages/companyInfoPackage/pages/comOperateStatus/zqxxDetail/main"
      });
    }
  },
  onPullDownRefresh() {
