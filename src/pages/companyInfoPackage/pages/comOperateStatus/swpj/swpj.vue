<template>
  <div class="default-bg min-height100">
    <ToDepthSearch
      :url="'/pages/companyInfoDepthPackage/pages/comOperateStatus/swpj/main'"
      v-if="total <= 0"
    ></ToDepthSearch>
    <div v-else>
      <DepthSearch :url="'/pages/companyInfoDepthPackage/pages/comOperateStatus/swpj/main'"></DepthSearch>
      ...
      <div class="block-shadow-content" :key="index" v-for="(item,index) in data">
        ...
        <div class="lh50">
          纳税评级：
          <span class="font-click">{{item.taxRating}}</span>
        </div>
        <div class="lh50">
          类型：
        ...
        <div class="lh50">
          纳税人识别号：
          <span class="font-click">{{item.taxpayerCode}}</span>
        </div>
        <div class="lh50">
          评价单位：
          <span class="font-click">{{item.evaluationUnit}}</span>
        </div>
      </div>
      <!--  -->
    ...
    init(init) {
      ...
      this.$http
        ...
        .then(res => {
          // console.log("res-->", res);
          if (res.data.code == 0) {
            const result = res.data.data.taxCredit;
            if (
