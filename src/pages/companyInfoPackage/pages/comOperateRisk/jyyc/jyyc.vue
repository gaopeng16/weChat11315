<template>
  <div class="default-bg min-height100">
    <ToDepthSearch
      :url="'/pages/companyInfoDepthPackage/pages/comOperateRisk/jyyc/main'"
      v-if="total <= 0"
    ></ToDepthSearch>
    <div v-else>
      <DepthSearch :url="'/pages/companyInfoDepthPackage/pages/comOperateRisk/jyyc/main'"></DepthSearch>
      <SubTitle :title="title" :subTitle="subTitle" :total="total"></SubTitle>
      <!--  -->
      <div class="block-shadow-content" :key="index" v-for="(item,index) in data">
        <BlockTitle :bt="bt" :title="item.putDate" :hasBt="true"></BlockTitle>
        <div class="lh50">
          列入原因：
          <span class="sub-content">{{item.putReason}}</span>
        </div>
        <div class="lh50">
          决定机关：
          <span class="sub-content">{{item.putDepartment}}</span>
        </div>
        <div class="lh50">
          移除日期：
          <span class="sub-content">{{item.removeDate}}</span>
        </div>
        <div class="lh50">
          移除原因：
          <span class="sub-content">{{item.removeReason}}</span>
        </div>
        <div class="lh50">
          移除机关：
          <span class="sub-content">{{item.putDepartment}}</span>
